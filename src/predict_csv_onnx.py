...
def group_metrics(df, group_col):
    ...
    for name, group in df.groupby(group_col, dropna=False):
        total = len(group)
        human_total = int((group["label"] == 0).sum())
        ai_total = int((group["label"] == 1).sum())
        pred_human = int((group["pred_label"] == 0).sum())
        pred_ai = int((group["pred_label"] == 1).sum())
        correct = int(group["correct"].sum())
        human_false_ai = int(((group["label"] == 0) & (group["pred_label"] == 1)).sum())
        ai_detected = int(((group["label"] == 1) & (group["pred_label"] == 1)).sum())
        ai_missed = int(((group["label"] == 1) & (group["pred_label"] == 0)).sum())
        ...
        rows.append(
            {
                ...
                "avg_prob_ai": float(group["prob_ai"].mean()) if total else None,
            }
        )
...
def parse_args():
    ...
    parser.add_argument("--input", required=True, help="Input CSV path. Must contain answer and label columns.")
...
def main():
    ...
    human_total = int((df["label"] == 0).sum())
    ai_total = int((df["label"] == 1).sum())
    if human_total:
        human_false_ai = int(((df["label"] == 0) & (df["pred_label"] == 1)).sum())
        print("human false positive rate:", f"{human_false_ai / human_total:.6f}", f"({human_false_ai}/{human_total})")
    if ai_total:
        ai_detected = int(((df["label"] == 1) & (df["pred_label"] == 1)).sum())
        print("ai recall:", f"{ai_detected / ai_total:.6f}", f"({ai_detected}/{ai_total})")

    for group_col in ["generator", "source"]:
        if group_col in df.columns:
            summary = group_metrics(df, group_col)
    ...
    preview_cols = [c for c in ["sample_id", "label", "generator", "source", "pred_label", "prob_ai", "correct"] if c in df.columns]
